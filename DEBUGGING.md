...
The bot uses several log prefixes. Filter with grep to isolate what you need:
...
# Gateway: CLOB auth, USDC balance, order submissions, rejections
tail -f /root/nitro-fig/logs/latest.log | grep GW

# Telemetry: locally rejected orders (insufficient balance, etc.)
tail -f /root/nitro-fig/logs/latest.log | grep TELEM
...
| `K` | `69076` | Strike price, set from Binance kline candle open for the market's interval |
...
- Gate result: `past_window` — blocked because this strategy only fires in the opening window (~5% of market duration: 15s for 5m, 120s for 1h, 300s for 4h)
...
| strike_misalign | `elapsed <= open_window` | VWAP available | `|dP| >= 0.02` | — |
...
| `lat=0.0ms` | Submit-to-ack latency (0.0ms in dry-run; 100-300ms in live CLOB) |
...
Two possible causes:
1. **Warmup not complete**: Each market requires 10 fresh one-second EWMA samples before trading is enabled. Check with `grep 'Warmup complete' /root/nitro-fig/logs/latest.log`. The log shows `warmup_baseline=N` and waits for 10 fresh samples beyond that baseline.
...
### Orders rejected locally (insufficient USDC)

The gateway checks available USDC before submitting to the CLOB. If the wallet has insufficient funds:
```
[TELEM] Order #5 rejected locally: insufficient USDC (latency_arb)
```

A Telegram alert is also sent. Fund the wallet with USDC.e on Polygon, or check balance with:
```bash
grep 'USDC' /root/nitro-fig/logs/latest.log
```

### CLOB authentication failed

```
[GW] CLOB authentication failed
```

Check that `POLYMARKET_PRIVATE_KEY` is set correctly in `.env`. The key must be the hex private key for a Polygon wallet. If using a proxy wallet, set `POLYMARKET_SIG_TYPE=1` and `POLYMARKET_FUNDER_ADDRESS`.

### No allowances set

```
[GW] WARNING: No USDC allowances set — run on-chain approve() first
```

Run the one-time approval: `cargo run --release --bin approve`. This approves USDC.e and CTF for all 3 exchange contracts.
...
- **Invalid token/chat_id**: Verify `TELEGRAM_BOT_TOKEN` and `TELEGRAM_CHAT_ID` in `.env`.
...
# Gateway: CLOB auth, balance, order submissions
grep GW /root/nitro-fig/logs/latest.log

# Locally rejected orders (balance, etc.)
grep 'rejected locally' /root/nitro-fig/logs/latest.log

# USDC balance tracking
grep 'USDC' /root/nitro-fig/logs/latest.log

# Raw CLOB request/response JSON (per market)
cat logs/1h/*/clob_raw.csv
